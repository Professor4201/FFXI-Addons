...
This addon has been preset with Maps,FFXI-HD,FFXI-Vision. Maps are marked up maps to show NM's, NPC's and other useful markers. FFXI-HD changes the quality of the graphics and FFXI-Vision is a  back up map repository from 2018, so moving on to quick install:

 Follow step 1) and then type in the chat box in game '//lua load XIPivot' after that type & Enter '//pivot status' to confirm addons are loaded.  
